...
import { SALES_SOURCE_KEYS, SALES_STATUSES } from "@/lib/sales";
...
function readCustomer(formData: FormData) {
  ...
  if (name.length > 180 || phone.length > 50 || email.length > 254 || notes.length > 5000 || sourceDetail.length > 180) throw new Error("Daxil edilən məlumatlardan biri çox uzundur.");
...
export async function createSalesCustomer(formData: FormData) {
  ...
  const { data: source } = await supabase.from("marketing_sources").select("id,key").eq("id", payload.source_id).eq("is_active", true).maybeSingle();
  ...
  if (source.key === "other" && !payload.source_detail) throw new Error("Digər mənbənin adını yazın.");
  if (source.key !== "other") payload.source_detail = null;
